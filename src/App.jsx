...
const sharedStyles = {
  ...
  nav:      { display: "flex", background: C.nav, borderBottom: `1px solid ${C.border}`, position: "sticky", top: 0, zIndex: 20 },
  ...
  return (
    <div style={{ display: "flex", gap: 8, alignItems: "center", padding: "5px 14px" }}>
      ...
      <input
        ...
        type="number" inputMode="decimal" placeholder="—" value={reps}
        onChange={e => onUpdate(exId, setIdx, "reps", e.target.value)}
      />
      <input
        style={{ background: "#111", border: `1px solid ${C.border}`, borderRadius: 6, color: C.text, flex: 1, padding: "8px 6px", textAlign: "center", fontSize: 14, ...mono }}
        type="number" inputMode="decimal" placeholder="—" value={weight}
            ...
            return (
              <div key={tpl.id} style={{ ...S.card, cursor: "pointer", transition: "border-color 0.15s" }}
                ...
                <div style={{ display: "flex", alignItems: "center", padding: "14px 14px 10px" }}>
                  ...
                  <div style={{ display: "flex", flexDirection: "column", alignItems: "center", marginLeft: 10, gap: 6 }}>
                    <span style={{ fontSize: 14, color: C.dim }}>›</span>
                    <div style={{ display: "flex", gap: 8 }}>
                      <button onClick={e => { e.stopPropagation(); editTpl(tpl); }} style={{ background: "none", border: "none", color: C.ghost2, fontSize: 12, padding: 2, ...mono }}>✎</button>
                      <button onClick={e => { e.stopPropagation(); deleteTpl(tpl.id); }} style={{ background: "none", border: "none", color: C.ghost2, fontSize: 14, padding: 2 }}
                        onMouseEnter={e => e.target.style.color = "#ef4444"} onMouseLeave={e => e.target.style.color = C.ghost2}>×</button>
                    </div>
                  </div>
                </div>
              </div>
